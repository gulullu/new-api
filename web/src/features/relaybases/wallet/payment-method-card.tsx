...
export function RelayBasesPaymentMethodCard({
  ...
  return (
    <Button
      ...
      className={cn(
        'group h-auto min-h-[76px] w-full min-w-0 justify-start gap-3 rounded-xl px-3.5 py-3 text-left whitespace-normal disabled:cursor-not-allowed disabled:opacity-100',
        belowMinimum && 'bg-muted/35 border-muted text-muted-foreground'
      )}
    >
      <span className='bg-background flex size-10 shrink-0 items-center justify-center rounded-lg border shadow-xs'>
        {loading ? (
          <Loader2 className='text-muted-foreground size-5 animate-spin' />
        ) : (
          getPaymentIcon(method.type, 'size-5', method.icon, method.name)
        )}
      </span>

      <span className='flex min-w-0 flex-1 flex-col items-start gap-1'>
        <span className='text-foreground w-full truncate text-sm font-semibold'>
          {method.name}
        </span>
        <span
          id={descriptionId}
          className='text-muted-foreground flex max-w-full items-start gap-1.5 text-xs leading-4 font-normal'
        >
          {channelHint === 'alipay' && (
            <SiAlipay
              aria-hidden='true'
