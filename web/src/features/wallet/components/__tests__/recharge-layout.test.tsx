...
describe('recharge form compact pricing layout', () => {
  ...
  test('shows original and sale prices without the legacy pay and savings copy', async () => {
    ...
    const price = card?.querySelector('[data-topup-preset-price]')
    ...
    assert.ok(card)
    assert.ok(price)
    assert.match(price.className, /flex-wrap/)
    assert.match(price.className, /items-baseline/)
    assert.match(price.className, /gap-x-2/)
    assert.equal(price.textContent?.trim(), 'Sale $12.83Original $14.25')
    assert.equal(original?.textContent?.trim(), 'Original $14.25')
    assert.equal(discounted?.textContent?.trim(), 'Sale $12.83')
    assert.match(discounted?.className ?? '', /font-semibold/)
    assert.match(original?.className ?? '', /text-\[11px\]/)
    assert.match(original?.className ?? '', /line-through/)
