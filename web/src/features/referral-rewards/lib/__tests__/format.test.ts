...
describe('referral reward formatting', () => {
  test('shows the processor-confirmed amount with its explicit currency', () => {
    const formatted = formatReferralPaidAmount('90', 'CNY', 'en')

    assert.equal(formatted.includes('CNY'), true)
    assert.equal(formatted.includes('90'), true)
  })
