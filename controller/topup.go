...
func GetTopUpInfo(c *gin.Context) {
	...
	qualifiedInvitees, err := model.GetQualifiedReferralInviteeCount(c.GetInt("id"))
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("查询有效受邀付费用户数失败 user_id=%d error=%q", c.GetInt("id"), err.Error()))
		qualifiedInvitees = 0
	}
	...
	data := gin.H{
		...
		"qualified_referral_invitees": qualifiedInvitees,
		// Deprecated aliases retained during the frontend rollout.
		"qualified_payments":  qualifiedInvitees,
		"qualified_referrals": qualifiedInvitees,
	}
...
func EpayNotify(c *gin.Context) {
	...
	payment, err := model.NewVerifiedPayment(verifyInfo.Money, "CNY", verifyInfo.TradeNo, verifyInfo.TradeNo, true)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 实付金额无效 trade_no=%s money=%q client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Money, c.ClientIP(), err.Error()))
		if _, writeErr := c.Writer.Write([]byte("fail")); writeErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), writeErr.Error()))
		}
		return
	...
	alreadyDone, err := model.RechargeVerifiedEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), payment)
	if err != nil {
		switch {
	...
	if alreadyDone {
		...
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值处理成功 trade_no=%s paid_amount=%s paid_currency=%s client_ip=%s", verifyInfo.ServiceTradeNo, payment.PaidAmountForLog(), payment.Currency, c.ClientIP()))
	}
	if _, writeErr := c.Writer.Write([]byte("success")); writeErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), writeErr.Error()))
	}
