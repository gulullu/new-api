...
var userGroupCatalog = map[string]groupTranslation{
	...
	"parnter":          groupDescription("db5e4883", "Stable GPT Pro pool for production.", "适合生产环境的稳定 GPT Pro 资源池。", "適合正式環境的穩定 GPT Pro 資源池。", "Pool GPT Pro stable pour la production.", "本番向けの安定した GPT Pro プール。", "Стабильный пул GPT Pro для продакшена.", "Nhóm GPT Pro ổn định cho môi trường sản xuất."),
	"claude-max":       groupDescription("7fe34730", "Pure Claude Max route for high-frequency production use; distillation is prohibited.", "Claude 纯 Max 路线，适合高频生产环境使用，禁止蒸馏。", "Claude 純 Max 路線，適合高頻正式環境使用；禁止蒸餾。", "Route Claude exclusivement Max, adaptée à un usage intensif en production ; distillation interdite.", "Claude Max 専用ルート。高頻度の本番利用向けで、蒸留は禁止されています。", "Маршрут только Claude Max для интенсивного использования в продакшене; дистилляция запрещена.", "Tuyến thuần Claude Max, phù hợp cho môi trường sản xuất tần suất cao; cấm chưng cất mô hình."),
	"claude-max-ultra": groupDescription("3ef7ac4e", "Pure Claude Max route for high-frequency production use; distillation is permitted.", "Claude 纯 Max 路线，适合高频生产环境使用，可蒸馏。", "Claude 純 Max 路線，適合高頻正式環境使用；可進行蒸餾。", "Route Claude exclusivement Max, adaptée à un usage intensif en production ; distillation autorisée.", "Claude Max 専用ルート。高頻度の本番利用向けで、蒸留が可能です。", "Маршрут только Claude Max для интенсивного использования в продакшене; дистилляция разрешена.", "Tuyến thuần Claude Max, phù hợp cho môi trường sản xuất tần suất cao; cho phép chưng cất mô hình."),
