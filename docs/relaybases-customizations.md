...
| --- | --- | --- | --- |
| Stripe 单位价与折扣 | Checkout 实付金额按 `充值额度 × StripeUnitPrice × 用户充值倍率 × 预设折扣` 计算；从已配置的一次性 Stripe Price 读取产品和币种，并以单行动态金额创建 Checkout。生产单位价以运行配置为准（当前目标为 `USD 0.15/Ɍ`），不应硬编码。中文一次性 Checkout 使用同一产品的 `CNY 1.00/Ɍ` 动态价格，并限制为支付宝；其他语言及订阅保持原逻辑。`TopUp.Money` 仍是入账基数，不能替换为实付金额。 | [controller/topup_stripe.go](../controller/topup_stripe.go)、[setting/payment_stripe.go](../setting/payment_stripe.go)、[setting/operation_setting/payment_setting.go](../setting/operation_setting/payment_setting.go) | 官方是否已支持折后实付、Price 校验和动态单行金额；Stripe API 的 Price/Checkout 字段是否变化。 |
| Waffo Pancake | 网关本体、Store/Product 绑定和 USD 单位价配置来自官方实现。中文钱包使用已绑定店铺下的专用人民币商品（`CNY 1.00/Ɍ`），其他语言继续使用 USD 商品；两条路径都把验签实付接入支付快照、推荐返利及退款撤销。运行配置需核对 `WaffoPancakeUnitPrice`、最低充值、Store、USD Product 和 CNY Product 的绑定。 | [controller/topup_waffo_pancake.go](../controller/topup_waffo_pancake.go)、[service/waffo_pancake.go](../service/waffo_pancake.go)、[setting/payment_waffo_pancake.go](../setting/payment_waffo_pancake.go) | 官方是否已提供等价的实付快照或返利钩子；Webhook 的 `total`、`currency`、`paymentId`、环境和退款事件契约是否变化。 |
...
| 合伙人佣金 | 独立 Partner 资格替代普通 3% 返利且不修改 API 计费分组：受邀用户每笔已验签生产 USD 或 CNY 充值，按该合伙人当时的专属比例（默认 30%）和真实实付计算佣金，不叠加普通返利、不追溯历史。Partner 账本始终以 USD micros 记账；CNY 订单使用建单时写入的不可变 `partner_settlement_usd_per_unit` 快照换算，并保留原始实付金额/币种。缺少或损坏快照时只跳过 Partner 佣金并回退标准首笔 3% 返利，绝不使用当前可变汇率。佣金以 USD micros 记账并保留比例快照，7 天后可转为 API Credits 或申请提现；最低提现 USD 20，同一时间仅一笔待处理。提现支持支付宝和 BNB Smart Chain（BEP-20）USDT，收款资料使用 `CRYPTO_SECRET`（未配置时为 `SESSION_SECRET`）派生密钥加密保存；该密钥在发布、重启和回滚时必须保持稳定。管理员二次验证后才可查看收款资料，并在实际打款后登记凭证。退款/争议撤销未结算或可用佣金；已使用部分形成持久调整额，由后续佣金抵扣。 | [model/partner_commission.go](../model/partner_commission.go)、[model/referral_reward.go](../model/referral_reward.go)、[controller/partner_commission.go](../controller/partner_commission.go)、[router/partner_router.go](../router/partner_router.go)、[web/src/features/partner-rewards](../web/src/features/partner-rewards) | 官方若新增多层级佣金或提现账本，迁移时必须保留 USD 精度、比例快照、7 天结算、提现幂等、退款负债和加密收款资料；不得与普通返利双发。 |
...
| 钱包付款方式品牌卡片 | 钱包付款卡片和确认弹窗恢复官方紧凑按钮、图标、标题和排列；仅在中文界面把 Stripe 的展示映射为支付宝、Waffo/Waffo Pancake 的展示映射为微信支付，提交时仍使用原始网关类型。非中文保持官方网关名称和图标。中文确认弹窗在 USD 应付金额下显示 `R 数量 × 折扣率` 的人民币近似值；支付方式与订阅区块继续上下排列。 | [web/src/features/relaybases/wallet/payment-method-card.tsx](../web/src/features/relaybases/wallet/payment-method-card.tsx)、[web/src/features/relaybases/wallet/policy.ts](../web/src/features/relaybases/wallet/policy.ts)、[web/src/features/wallet/components/dialogs/payment-confirm-dialog.tsx](../web/src/features/wallet/components/dialogs/payment-confirm-dialog.tsx)、[web/src/features/wallet/lib/ui.tsx](../web/src/features/wallet/lib/ui.tsx) | 使用系统 Epay 编辑器同源的 `SiAlipay`/`SiWechat` 图标；Waffo/Pancake 下单优先传账户邮箱。保留官方 Waffo 子方式的根相对图标路径；Waffo Pancake SDK 当前没有可安全传入付款方式的字段，不伪造 WeChat 参数。Stripe 仅在中文一次性 Checkout 指定 `alipay`，订阅和其他语言保持原逻辑。 |
...
| `partner_settlement_usd_per_unit` | CNY 订单创建时锁定的“每个支付单位对应 USD”结算基准；仅供 Partner USD 账本换算，USD 订单为空 | 否 |
...
- 支持人民币 Checkout 的新订单同时记录 `partner_settlement_usd_per_unit`；它与支付快照一并属于订单级不可变结算依据，不随后台后续改价变化。Stripe/Waffo/Waffo Pancake 的 CNY 订单写入对应 USD 单位价；Epay 旧式 CNY 回调没有可靠的订单级 USD 基准，Partner 佣金会安全跳过并回退标准首笔返利。
...
| `top_ups.referral_unit_price` | Stripe/Waffo Pancake 下单时的 USD 单位价快照，仅供普通返利换算；不得参与支付结算或充值入账。历史空值不猜测回填。 |
| `top_ups.partner_settlement_usd_per_unit` | 支持 CNY Checkout 的 Stripe/Waffo/Waffo Pancake 订单级 USD 换算快照，仅供 Partner USD 佣金；空值或非法值不使用当前配置猜测。Epay CNY 无可靠快照时跳过 Partner 并走标准首笔返利。 |
...
| 合伙人佣金 | 专属比例快照、同受邀用户多笔付款均发放、与普通 3% 互斥、CNY→USD 订单快照换算、缺失快照安全降级、7 天边界结算、退款前后冲正、已转出后形成调整额、USD→Credits 单价快照、最低 USD 20、单笔待处理提现、支付宝/BSC 校验、收款资料密文与二次验证、管理员打款/驳回幂等；MySQL/PostgreSQL 并发双回调仅一笔账。 |
...
| 2026-08-27 | `v1.0.0-rc.26-relaybases.37` | 钱包中文支付图标改用系统 Epay 的黑色 `SiAlipay`/`SiWechat`；确认弹窗人民币近似值改为 `R 数量 × 折扣率`；Stripe、Waffo 与 Waffo Pancake 优先传账户邮箱，空邮箱保留 Waffo 兼容回退 | 无 | Stripe Checkout、Waffo/Waffo Pancake | 受影响前端测试、七语言覆盖、Go Stripe/Waffo 邮箱测试、S12 隔离 typecheck/lint/build；发布时只重建 new-api，保留上一版镜像回滚标签 |
...
| 2026-08-28 | `v1.0.0-rc.26-relaybases.48` | 统一多币种账务语义：订单保留原币种/实付金额，新增 CNY→Partner USD 的订单级结算快照；Stripe/Waffo Pancake 中文 Checkout 使用人民币商品/价格；合伙人说明改为默认展开的图标卡片并同步七语言；VIP 付款提醒精简为短文案 | `top_ups.partner_settlement_usd_per_unit`（AutoMigrate，旧订单为空安全读取） | Stripe Checkout、Waffo/Waffo Pancake | Partner/TopUp/Controller 聚焦 Go 测试、前端组件/i18n 测试、S12 typecheck/lint/build；只重建 `new-api`，保留 `.47` 回滚镜像 |
| 2026-08-28 | `v1.0.0-rc.26-relaybases.49` | 英文界面 Waffo 支付方式补齐仓库内置明/暗色品牌图标；旧配置中的无效或缺失图标安全回退，合法的子支付方式图片保持原样 | 无 | Waffo、Waffo Pancake | Waffo 钱包组件/图标聚焦测试、S12 隔离 typecheck/lint/build；只重建 `new-api`，保留 `.48` 回滚镜像 |
| 2026-08-30 | `v1.0.0-rc.26-relaybases.50` | 修复非中文界面 Waffo Pancake 在生产旧图标名 `LuGlobe2` 无法解析时显示为空；卡片与确认弹窗统一使用系统内置明/暗色 Waffo 品牌图标，保持与 Stripe 同级的尺寸和布局 | 无 | Waffo Pancake | 生产配置回读、失败用例复现、卡片/确认弹窗聚焦测试、S12 隔离 typecheck/lint/format/build；只重建 `new-api`，保留 `.49` 回滚镜像 |
