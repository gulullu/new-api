...
| 钱包付款方式品牌卡片 | Stripe/Waffo 使用 feature-owned 品牌图标、加粗边界与轻量层次；默认态可识别，选中/报价态通过品牌边框、焦点环、勾选或加载状态同步强化。Waffo 与 Waffo Pancake 的所有钱包和付款确认入口都优先使用仓库内静态品牌资源，不依赖后台可配置的异步图标名称；移动端保持单列，宽屏双列。 | [web/src/features/relaybases/wallet/payment-method-card.tsx](../web/src/features/relaybases/wallet/payment-method-card.tsx)、[web/src/features/wallet/lib/ui.tsx](../web/src/features/wallet/lib/ui.tsx)、[web/public/waffo-logo-dark.svg](../web/public/waffo-logo-dark.svg)、[web/public/waffo-logo-light.svg](../web/public/waffo-logo-light.svg) | 官方若提供支付卡品牌 slot，迁移到 slot 后删除宿主的窄 props 接线；不得重新让 Waffo 品牌图标依赖运行配置或远程资源。 |
